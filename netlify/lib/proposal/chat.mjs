...
/* 끝맺음 패턴은 물음표·느낌표·말줄임이 붙어도 잡혀야 한다 — 「어려운가요?」가 되묻기로 떨어졌다(라이브 2026-09-19) */
...
const FILLER = /(글쎄|모르겠|모름|음\.\.|그게|그거|저거|요거|그니까|그러니까|저기요|뭐라고|뭐지|뭐더라|있잖|아직|비밀|나중에|몰라|없어요|없습니다|아무거나|테스트|test|ㅋㅋ|ㅎㅎ|ㅁㄴㅇ|ㅋㅋㅋ|\.\.\.)/i;
...
export function ruleParse(text, asking, fields = {}) {
  ...
  const plain = clip(KO.stem(raw
    ...
    /* 말머리 부사는 이름의 일부가 아니다 — 「그냥 공장이에요」 → 「공장」 */
    .replace(/^(그냥|그저|뭐|음|어|아|저기|일단|그러니까|그니까)\s+/, '')
  ), 60);

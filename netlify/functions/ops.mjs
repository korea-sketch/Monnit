...
export default async (req) => {
  ...
  if (sub === 'testmail') {
    ...
    const r = await notify(
      { ts: now, label: '발송점검', channel: '직접', point: '/ops/testmail',
        company: '(발송 점검)', name: '-', phone: '-', email: '',
        interest: '알림 메일 발송 점검',
        memo: '이 메일이 도착했다면 서버 알림 경로는 정상입니다.',
        source: 'ops', landing: '/ops' },
      { '점검 시각': now }
    );
    return j({ sent: r.ok, detail: r });
  }

  /* 누락분 복구 — 기간 안의 접수를 다시 메일로 보내고 먼데이에 올린다.
     이미 올라간 건은 _monday.mjs 가 걸러낸다. 메일은 dry 로 미리 볼 수 있다. */
  if (sub === 'resend') {
    const q = new URL(req.url).searchParams;
    const from = q.get('from') || '', to = q.get('to') || '';
    const dry  = q.get('dry') === '1';
    const okDay = v => /^\d{4}-\d{2}-\d{2}$/.test(v);
    if (!okDay(from) || !okDay(to) || from > to)
      return j({ error: 'from · to 를 YYYY-MM-DD 로 주세요' }, 400);

    const now = new Date();
    const keys = [];
    for (let i = 0; i <= 12; i++) keys.push(monthKey(new Date(now - i * 30.5 * 864e5)));
    let rows = [];
    for (const k of [...new Set(keys)]) rows = rows.concat(await readLines('leads', k));
    rows = rows.filter(r => r?.ts && !/^__/.test(r.company || ''));
    rows = rows.filter(r => { const d = kday(new Date(r.ts)); return d >= from && d <= to; });
    rows.sort((a, b) => String(a.ts).localeCompare(String(b.ts)));

    const idOf = r => (r.ts || '') + '|' + (r.email || r.phone || r.company || '');
    const list = rows.map(r => ({
      id: idOf(r), ts: r.ts, 회사: r.company || '', 담당: r.name || '',
      연락처: r.phone || '', 이메일: r.email || '', 관심: r.interest || '', 채널: r.channel || ''
    }));
    if (dry) return j({ from, to, count: rows.length, dry: true, leads: list });

    /* 한 통으로 묶어 보낸다 — 20통이 따로 오면 오히려 놓친다 */
    let mail = { ok: false, skipped: '보낼 건 없음' };
    if (rows.length) {
      const body = rows.map((r, i) =>
        `${i + 1}. ${r.company || r.name || r.email || '(미식별)'}\n` +
        `   담당 ${r.name || '-'} · ${r.phone || '-'} · ${r.email || '-'}\n` +
        `   ${r.interest || '-'} · ${r.channel || '-'} · ${kday(new Date(r.ts))}`
      ).join('\n\n');
      mail = await notify(
        { ts: new Date().toISOString(), label: '누락 복구', channel: '직접', point: '/ops/resend',
          company: `${from} ~ ${to} 누락 접수 ${rows.length}건`, name: '', phone: '', email: '',
          interest: '알림이 나가지 않았던 접수 모음', memo: body, source: 'ops', landing: '/ops' },
        {}
      );
    }

    const pushed = [];
    for (const r of rows) pushed.push({ id: idOf(r), ...(await pushLead(idOf(r), r)) });

    return j({
      from, to, count: rows.length,
      mail: mail.ok, mailDetail: mail,
      monday: {
        created: pushed.filter(p => p.ok && !p.skipped).length,
        already: pushed.filter(p => p.skipped === '이미 등록됨').length,

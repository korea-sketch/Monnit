...
let SITE_CONTENT = {};
const SITE_VIEWS = ['view-home','view-who-we-are','view-what-we-do'];
...
function eachSlot(cb){
  SITE_VIEWS.forEach(function(vid){
    ...
    if (vid === 'view-home'){
      // 메인페이지는 이름표가 붙은 슬롯(data-sc)만 편집 대상 — 동적 카드 영향 없이 안정적
      root.querySelectorAll('[data-sc]').forEach(function(el){
        cb(el.getAttribute('data-sc'), el, el.tagName === 'IMG' ? 'img' : 'text');
      });
    } else {
...
function mapSiteContent(rows){
  const out = {};
  rows.forEach(function(o){ const k=(o.key||'').trim(); if(!k) return; out[k]={ ko:o.ko||'', en:o.en||'', image:o.image||'' }; });
  return out;
}
function applySiteContent(){
  if(!SITE_CONTENT || !Object.keys(SITE_CONTENT).length) return;
  let lang='ko'; try{ lang=localStorage.getItem('mlang')||'ko'; }catch(e){}
  eachSlot(function(key, el, kind){
    const row = SITE_CONTENT[key]; if(!row) return;
    if(kind==='img'){
      if(row.image && row.image.trim()){ const u=normalizeImageUrl(row.image.trim()); if(el.getAttribute('src')!==u) el.setAttribute('src',u); }
    } else {
      const val = (lang==='en' && row.en && row.en.trim()) ? row.en : row.ko;
      if(val!=null && String(val).trim()!=='' && el.innerHTML!==val) el.innerHTML=val;
    }
  });
  // SEO: seo#title → 문서 제목, seo#description → meta description
  (function(){
  ...
  // 카탈로그 링크: SiteContent의 product#catalog 행(ko=국문 URL, en=영문 URL)로 모든 '카탈로그 다운로드' 버튼 주소를 시트에서 변경 가능하게
  (function(){
...
async function loadSheetData(){
  ...
  add('logos',         r => { const m = mapLogos(r);         renderLogos(m); });
  add('sitecontent',   r => { SITE_CONTENT = mapSiteContent(r); applySiteContent(); });
...
function renderLogos(list){
  if(!list || !list.length) return; // 시트가 비어있으면 index.html의 기본 로고를 그대로 둔다
  ...
  const slide = it => {
    ...
    const img = src ? '<img src="'+esc(src)+'" alt="'+esc(it.name)+'" loading="lazy">' : '';
    return '<div class="logo-slide">'+img+'<span>'+esc(it.name)+'</span></div>';
  };
  const html = list.map(slide).join('');
  track.innerHTML = html + html; // 무한 스크롤(마퀴)용 2배 복제
}

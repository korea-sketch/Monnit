...
(function (w, d) {
  ...
  var CSS = [
    ...
    /* ── 1단계 · 하단 슬림 배너 ─────────────────────────────────
       한 줄 레이아웃: 문구(좌) · 링크 + 버튼(우). 높이를 최소화해
       첫 화면 콘텐츠를 거의 가리지 않습니다. */
    ...
    '.mnk-cc.bar .mnk-cc-bar{pointer-events:auto;width:100%;max-width:none;background:rgba(0,27,36,.96);color:#eef7f8;',
    'backdrop-filter:blur(8px);border-top:1px solid rgba(0,201,192,.24);box-shadow:0 -8px 24px rgba(0,0,0,.35);',
    'padding:12px 22px;transform:translateY(100%);transition:transform .3s cubic-bezier(.22,1,.36,1)}',
    '.mnk-cc.bar.show .mnk-cc-bar{transform:none}',
    '.mnk-cc-bar-in{max-width:1240px;margin:0 auto;display:flex;align-items:center;gap:18px;flex-wrap:wrap}',
    '.mnk-cc-bar p{flex:1 1 380px;font-size:13px;line-height:1.55;color:#b9ced2;margin:0;word-break:keep-all}',
    '.mnk-cc-bar .lnks{display:flex;gap:14px;flex:0 0 auto}',
    '.mnk-cc-bar .lnks a,.mnk-cc-bar .lnks button{background:none;border:0;padding:0;font-family:inherit;cursor:pointer;',
    'font-size:12.5px;color:#4dd0c7;text-decoration:none;white-space:nowrap}',
    '.mnk-cc-bar .lnks a:hover,.mnk-cc-bar .lnks button:hover{text-decoration:underline}',
    '.mnk-cc-bar .acts{display:flex;gap:8px;flex:0 0 auto}',
    '.mnk-cc-bar .acts button{padding:8px 16px;border:0;border-radius:3px;font-family:inherit;font-size:13px;',
    'font-weight:700;cursor:pointer;white-space:nowrap;transition:.15s}',
    '.mnk-cc-bar .acts button[data-act="necessary"]{background:transparent;color:#9fb6ba;',
    'border:1px solid rgba(255,255,255,.18)}',
    '.mnk-cc-bar .acts button[data-act="necessary"]:hover{color:#fff;border-color:rgba(255,255,255,.38)}',
    '.mnk-cc-bar .acts button[data-act="all"]{background:#00c9c0;color:#00232b}',
    '.mnk-cc-bar .acts button[data-act="all"]:hover{background:#2ee0d7}',
    '@media(max-width:820px){.mnk-cc-bar{padding:12px 16px}.mnk-cc-bar-in{gap:10px}',
    '.mnk-cc-bar p{flex:1 1 100%;font-size:12.5px}.mnk-cc-bar .lnks{flex:1 1 auto}',
    '.mnk-cc-bar .acts{flex:1 1 auto;justify-content:flex-end}.mnk-cc-bar .acts button{flex:1 1 0;padding:9px 10px}}'
  ].join('');
  ...
  function banner() {
    var el = shell(
      '<p>사이트 개선·이용 분석·광고 성과 측정을 위해 쿠키를 사용합니다. ' +
      '수집 정보는 미국(Google · Microsoft)으로 이전될 수 있으며, 동의는 언제든 철회할 수 있습니다.</p>' +
      '<div class="lnks">' +
        '<a href="' + PRIVACY_URL + '" target="_blank" rel="noopener">쿠키 공지</a>' +
        '<button type="button" data-act="settings">설정</button>' +
      '</div>' +
      '<div class="acts">' +
        '<button type="button" data-act="necessary">거부</button>' +
        '<button type="button" data-act="all">모두 수락</button>' +
      '</div>',
      true   /* 하단 배너 모드 */
    );

...
(function () {
  ...
  function autoMount(root) {
    ...
    Array.prototype.forEach.call(nodes, function (el) {
      ...
      /* 2026-09-17 — 화면에 보일 때 처음 만든다. 홈에서 숨어 있는 About 뷰까지
         WebGL 셰이더를 컴파일하느라 첫 화면이 늦어지던 것(모바일 약 1.7초)을 줄인다. */
      if (typeof IntersectionObserver === 'function' && !el.__galaxyWait && !(el.offsetWidth || el.offsetHeight)) {
        el.__galaxyWait = true;
        var lazy = new IntersectionObserver(function (ents) {
          if (!ents.some(function (en) { return en.isIntersecting; })) return;
          lazy.disconnect(); el.__galaxyWait = false; autoMount(el.parentNode || document);
        }, { rootMargin: '200px' });
        lazy.observe(el);
        return;
      }
      if (el.__galaxyWait) return;
      var custom = {};
